...
receivedflag1=0
globaltime=0
stats = open('assertion_collision_speed.txt','a')
fileno = 0
global coll1
global coll2
...
class Gazebo_check1(smach.State):
    ...
    def execute(self, userdata):
        ...
        if spd<0.25:
		global stats
		stats.write('Assertion_collision_speed at trace ' + str(fileno) +': True at global time '+ str(time.time()-globaltime) +'\n')
        else:
        	global stats
		stats.write('Assertion_collision_speed at trace ' + str(fileno) +': False at global time '+ str(time.time()-globaltime) +'\n')
...
def main(number):
	rospy.init_node('assertion_collision_speed', anonymous=True) #Start node first
	global globaltime
